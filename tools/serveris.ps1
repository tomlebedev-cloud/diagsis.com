# Paprastas vietinis serveris peržiūrai: http://localhost:8098
$saknis = Split-Path -Parent $PSScriptRoot
$l = New-Object System.Net.HttpListener
$l.Prefixes.Add("http://localhost:8098/")
try { $l.Start() } catch { Write-Host "NEPAVYKO: $($_.Exception.Message)"; exit 1 }
Write-Host "Serveris veikia: http://localhost:8098  (Ctrl+C sustabdyti)"
